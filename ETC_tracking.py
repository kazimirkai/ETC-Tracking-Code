...
# ----------------------------------------------------------
all_files = set()                                                                    #creates a set for files to go into 
current = start_date                                                                 #defines start date maker based on value given earlier 
while current <= end_date:                                                           #while marker is before end date (given earlier)...
    year = current.strftime('%Y')                                                         #extracts the year value from the netcdf files (given in ISO 8601 format) using numpy's datetime library
    matched_files = glob.glob(f"{data_dir}/{year}/*{current.strftime('%Y-%m')}*.nc")      #searches/compiles files from the year/month of the marker in the filename
    all_files.update(matched_files)                                                       #add files to prelabelled set 
    current += timedelta(days=1)                                                          #adds a day to the marker 

all_files = sorted(all_files)                                                       #sort files by date by default due to numerical nature of the netcdffile naming system 
if not all_files:                                                                   #if there are no files 
    raise FileNotFoundError("No NetCDF files found for the specified date range.")  #say that none were found in the directory specified earlier 

ds = xr.open_mfdataset(all_files, combine='by_coords', engine='netcdf4')            #reads all files and labels resulting xarray dataset, specifying the file type (netcdf)  
...
# -------------------------------------
# Tracking Section with progress report 
# -------------------------------------
...
for t in range(total_steps):                                            #for each time step, t 
    ...
    for c in coords:                                                 #for each local pressure minimum 
        ...
        dy = np.abs(ys - SN)                                         #calculate distance in lat/lon to min 
        dx = np.abs(xs - WE)
        
       
        neighborhood = slp_snapshot.where((dy <= neighborhood_m) & (dx <= neighborhood_m), drop=True)   #get distances from min to all other grid pts  
        
                                     
        if neighborhood.mean().values - pval > min_pressure_diff:     #If distance less than neighborhood distance, acquire all those coords 
...
proj_lcc = Proj(proj='lcc', lat_1=33, lat_2=45, lat_0=36.775, lon_0=-97)         #specify and define paremeters of projection to use 
...
for tid in track_ids:                                                     #for each track (id) 
    ...
    points = track_df[['lon','lat']].to_numpy().reshape(-1,1,2)           #define the points in the track as an array of the latitude and longitudes                                                    #move to next track
...
plt.savefig(output_id_png, dpi=300, bbox_inches='tight')                #save in appropriate directoty with 300 dpi size, small borders
...
cmap_slp = plt.get_cmap('viridis_r')                                              #Establish colormap used 

# Compute segment mean pressures for normalization
segment_slps = []                                                                 #create empty array for line segments 
for tid, track_df in df.groupby('track_id'):                                      #extract all rows for each track id 
    slp_vals = track_df['slp'].to_numpy()                             #highlight slp values from those rows, convert from dataframe to numpy array  
    segment_slps.extend(0.5 * (slp_vals[:-1] + slp_vals[1:]))         #calculating/defining mean pressure for each segment from slp array (for colormap)

segment_slps = np.array(segment_slps)                                              #putting values into a numpy array 
norm_slp = mpl.colors.Normalize(vmin=segment_slps.min(), vmax=segment_slps.max())  #normalize the number of shades on the colormap to the range of mean slp values in the track

# Create figure
fig = plt.figure(figsize=(10,8))                                                   #define figure, size 
ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())                       #create subplot

# Map features
ax.set_extent([-100, -50, 20, 65], crs=ccrs.PlateCarree())
ax.add_feature(cfeature.COASTLINE)
ax.add_feature(cfeature.BORDERS)
ax.add_feature(cfeature.STATES, linewidth=0.4)                             #define extent, projection, and aesthetic features/colors of map 
ax.add_feature(cfeature.LAND, facecolor='lightgray', alpha=0.5)
ax.add_feature(cfeature.OCEAN, facecolor='lightblue', alpha=0.3)

# Plot each track
for tid, track_df in df.groupby('track_id'):                           #begin for loop for each track id

    points = track_df[['lon','lat']].to_numpy().reshape(-1,1,2)    #define numpy array of points in between segments from lat/lon
    segments = np.concatenate([points[:-1], points[1:]], axis=1)       #define segments 

    slp_vals = track_df['slp'].to_numpy()                    #putting values into a numpy array 
    segment_slp = 0.5 * (slp_vals[:-1] + slp_vals[1:])       #calculate mean pressure of track for each segment (for plotting)  

    lc = LineCollection(
        segments,
        cmap=cmap_slp,                 #establish line collection with segments, colormap used, number of shades, linewidth, and projection 
        norm=norm_slp,
        linewidths=2.0,
        transform=ccrs.PlateCarree()
    )
    lc.set_array(segment_slp)                          #create an id of slp values for segments in the tracks of the linecollection 
    ax.add_collection(lc)                              #add line collection to plot 

ax.scatter(
    df['lon'],
    df['lat'],
    c='k',                           #add scattered points to plot, color:black, size 15, opacity 0.6, projection, stacking behavior 
    s=15,
    alpha=0.6,
    transform=ccrs.PlateCarree(),
    zorder=3
...
# Colorbar
sm = mpl.cm.ScalarMappable(cmap=cmap_slp, norm=norm_slp)            #define scalarmappable object and array from colormap and number of shades to map values onto the colorbar
sm.set_array([])                                                           
cbar = fig.colorbar(sm, ax=ax, orientation='vertical')              #create colorbar using scalaramappable object, specify orientation/location
cbar.set_label('Segment Mean SLP (hPa)')

plt.title("Tracked Low-Pressure Centers — Colored by Mean Segment SLP")     #title plot 
plt.savefig(output_slp_png, dpi=300, bbox_inches='tight')                   #save in appropriate directoty with 300 dpi size, small borders
plt.close()                                                                 #close plot creation

print(f"Saved slp plot: {output_slp_png}")                                  #announce that the plot was created 

# ----------------------------
# Track Density Contour Plot
# ----------------------------
#setting parameters
smoothing_sigma = 6.0                                                      #smoothing level                 
grid_resolution = 200                                                      #resolution of smoothing grid 
density_cmap = "Blues"                                                     #colormap used
buffer_km_density = 300                                    #specifies buffer near map boundaries to avoid unnatural looking density maps 
num_contours = 16                                                          #number of contours in final plot 

x_all = df['lon'].values                                                   #acquire x/y of the tracking output 
y_all = df['lat'].values

x_min_d, x_max_d = x_all.min(), x_all.max()                                #find/label edge coordinates
y_min_d, y_max_d = y_all.min(), y_all.max()

lat_buffer = buffer_km_density / 111.0                                     #create lat buffer by converting km to geographic coords
mean_lat = y_all.mean()                                                    #calculate average latitude 
lon_buffer = buffer_km_density / (111.0 * np.cos(np.deg2rad(mean_lat)))    #create lon buffer by converting km to geographic coords, making 
                                                                        #adjusting km - degree conversion with average latitude calculation 
x_min_d -= lon_buffer                                                      
x_max_d += lon_buffer
y_min_d -= lat_buffer                                                      #expand all domain edges by buffer 
y_max_d += lat_buffer

x_grid = np.linspace(x_min_d, x_max_d, grid_resolution)                   #create arrays of coordinates for specified grid resolutions
y_grid = np.linspace(y_min_d, y_max_d, grid_resolution)

H, _, _ = np.histogram2d(y_all, x_all, bins=grid_resolution, range=[[y_min_d, y_max_d],[x_min_d, x_max_d]])    #density calculation (calculate how many track points from 
H_smooth = gaussian_filter(H, sigma=smoothing_sigma)       #previously aquired lat/lon points fall into the bins formed by the grid resolution between the min and max lats/lons using a 2d histogram)
                                            #apply gaussian smoothing from the initially defined value


X, Y = np.meshgrid(x_grid, y_grid)                               #create 2d coorinate array                      
levels = np.linspace(0, 0.15, num_contours)                      #create contour values from contour number and min/max values 

fig = plt.figure(figsize=(10,8))                                 
ax = plt.axes(projection=ccrs.PlateCarree())                  
ax.set_extent([-100, -50, 20, 65], crs=ccrs.PlateCarree())      
ax.add_feature(cfeature.COASTLINE)                               #define extent, projection, and aesthetic features/colors of map 
ax.add_feature(cfeature.BORDERS)
ax.add_feature(cfeature.STATES, linewidth=0.4)
ax.add_feature(cfeature.LAND, facecolor='lightgray', alpha=0.5)
ax.add_feature(cfeature.OCEAN, facecolor='lightblue', alpha=0.3)

cf = ax.contourf(X, Y, H_smooth, levels=levels, cmap=density_cmap, norm=mpl.colors.PowerNorm(gamma=0.5), extend='max')    #create density contour using gamma 0.5 to boost low density features allowing for values above max contour 
ax.contour(X, Y, H_smooth, levels=levels, colors='k', linewidths=0.5)                   #draw lines at edges of contours to make them more visible 

cbar = plt.colorbar(cf, ax=ax, orientation='vertical', fraction=0.046, pad=0.04)        #add colorbar to plot 
cbar.set_label("Track Density (smoothed)")                                              #label colorbar 

plt.title("Track Density")
plt.savefig(output_density_png, dpi=300, bbox_inches='tight')            #title plot and save to correct directory with appropriate resolution, border sizes
plt.close()

print(f"Saved density plot: {output_density_png}")                       #announce that the plot was created 
